...
import { setChains, setWalletChainId } from '../background/storage';
...
describe('Background message router', () => {
  ...
  it('WALLET_CHAIN_CHANGED caches the chain ID', async () => {
    ...
    // The cached value should be retrievable via storage.
    expect(await setWalletChainId(137)).toBeUndefined(); // set returns void
  });
  ...
  it('PROVIDER_REQUEST_INTERCEPTED for chain switch to configured chain is LOW', async () => {
    await setChains([ETH]);
    const res = (await handleBackgroundMessage({
  ...
  it('PROVIDER_REQUEST_INTERCEPTED for chain switch to unknown chain is MEDIUM', async () => {
    await setChains([ETH]);
    const res = (await handleBackgroundMessage({
      type: MESSAGE_TYPES.PROVIDER_REQUEST_INTERCEPTED,
      requestId: 'r2',
      ...
      origin: 'https://example.com',
      chainId: 1,
    })) as { type: string; analysis: { riskLevel: string } };
    // 0x1a4 = 420 (unknown chain) -> MEDIUM.
    expect(res.analysis.riskLevel).toBe('MEDIUM');
  });

  it('unknown message type returns undefined', async () => {
    const res = await handleBackgroundMessage({ type: 'UNKNOWN_TYPE' });
